...
@export var max_boost_multiplier = 1.4
...
func _input(event):
	if event is InputEventMouseMotion && Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		rotate_y(deg_to_rad(event.relative.x * mouse_y_sens * -1))
		$UpperCollider/Camera.rotate_x(deg_to_rad(event.relative.y * mouse_x_sens * -1))
...
func _process_input(_delta):
	...
	if Input.is_action_just_pressed("crouch"):
		if on_floor:
			crouch_floor = true
		crouching = true
		tween.tween_property($LowerCollider, "position", Vector3(0,.25, 0), .1)
		tween.stop()
		inbetween = true
...
#			if input_dir.length() > .1:
	...
	if state == State.RUN:
		...
		velocity.y = ((Vector3(velocity.x, 0, velocity.z).dot(collision.get_normal())) * -1)
	...
	if collision:
		if Vector3.UP.dot(collision.get_normal()) < .5:
			...
			velocity = Vector3(velocity.x, velocity.y + ((Vector3(velocity.x, 0, velocity.z).dot(collision.get_normal())) * - 2) , velocity.z)
		else:
			velocity = velocity
...
func _update_hud():
	...
	if cursor_object == null:
		$HUD/Crosshair.material.set("shader_parameter/color_id", 0)
	elif cursor_object.is_in_group("enemy"):
		$HUD/Crosshair.material.set("shader_parameter/color_id", 1)
	elif cursor_object.is_in_group("friend"):
		$HUD/Crosshair.material.set("shader_parameter/color_id", 2)
	else:
		$HUD/Crosshair.material.set("shader_parameter/color_id", 0)
	
	$HUD/Crosshair.material.set("shader_parameter/spread", velocity.length()/4 + 1)
